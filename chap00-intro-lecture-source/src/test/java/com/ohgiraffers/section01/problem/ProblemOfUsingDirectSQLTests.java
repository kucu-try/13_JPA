...
class ProblemOfUsingDirectSQLTests {
    ...
    * 1. 데이터 변환 , SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지 보수정 저하)
    * 2. SQL에 의존하여 개발
    ...
    * */
    ...
    @Test
    void testDirectionSelectSql() throws SQLException {
        //given
    ...
    /*
    * 2. SQL에 의존하여 개발
    * 요구사항의 변경에 따라 애플리케이션의 수정의 SQL의 수정으로도 이어진다
    * 이러한 수정 영향을 미치는 것은 오류를 발생시킬 가능성도 있지만 유지 보수성에도 악영향을 ㄱ미친다
    * 또한 객체를 사용할 때 SQL 에 의존하면 객체에 값이 무엇이 들어있는지 확인하기 위해 SQL을 매번 살펴야한다
    * */
    ...
    /*
    *  3. 패러다임 불일치(상소그 연관관계, 객체 그래프 탐색)
    * 3-1 상속 문제
    * 객체 지향 언어의 상속 개념과 유사한 것이 데이터 베이스의 서브타입엔티티이다
    * 유사한 것 같지만 다른 부분은 데이터베이스의 상속은 상속 개념을 데이터로 추상화해서 슈퍼타입과 서브타입으로 구분하고,
    * 슈퍼타입의 모든 속성을 서브타입이 공유하지 못하며 물리적으로 다른 테이블로 분리가 된 형태이다
    * (설계에 따라서는 속성으로 추가되기도 한다)
    * 하지만 객체지향의 상속은 슈퍼타입의 속성을 공유해서 사용하므로 여기서 패러다임 불일치 현상이 발생한다
    *
    * 법인 회원과 일반회원이라는 두 가지 회원을 추상화하여 회원이라는 슈퍼타입 엔터티를 도출하고
    * 서브 타입 엔터티로 법인회운과 일반회원이 존재하는 상황인 경우
    * 물리적으로 회원과 법이회원 테이블로 분리된 상황에서는 각각 insert 구문을 수행해야 한다
    * insert into 회원
    *  insert into 법인회원
    ...
    *  * public class Menu {
    ...
    void testEquals() throws SQLException {
        ...
        Assertions.assertEquals(menu,menu2); // 주소의 값이 달라서 값이 같아도 false가 나옴
        //이거 말고 jpa에서는 영속성 컨테스트로 인해 true 가 나온다
