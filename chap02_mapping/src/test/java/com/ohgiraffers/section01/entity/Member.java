 ...
 * 또한 name 속성을 사용하여 엔티티 클래스와 매핑될 테이블의 이름을 지정할수 있다
 * 생략하면, 자동으로 클래스 이름을 엔티티명으로 사용한다
 *
 * 프로젝트 내에 다른 패키지에도 동일한 엔티티가 존재하는 경우 해당 엔티티를 식별하기 위한 중복되지 않는 name을 지정해주어야한다
 * 기본 생성자는 필수로 작성해야한다
 * final 클래스 enum, interface , inner class 에서는 사용불가능이다
 * 저장할 필드에 final을사용하면 안된다.
 * */
