...
public class EmbeddedKeyTests {
    ...
    /*
    * jpa에서 복합 키를 매핑하는 두 가지가 있다
    * 첫 번째 @EmbeddedId 어노테이션을 사용하는 방법이다
    * 이 방법은 복합 키를 구성하는 필드들을 하나의 클래스로 묶은 뒤, 해당 클래스를 @EmbeddedId 어노테이션을 사용하여 매핑하는 것이다
    * 이 방법은 복합 키의 일부 필드만을 매핑할 수도 있기 떄문에, 필드 수가 많은 경우에는 유연한 매핑이 가능하다는 장점이 있다
    * 두 번째는 @IdClass 어노테이션을 사용하는 방법이다.
    * 이 방법은 복합 키를 구성하는 필드들을 별도의 클래스로 분리한 뒤, 해당 클래스를 @IdClass 어노테이션의 값으로 지정해주는 것이다
    * 이 방법은 복합 키를 구성하는 모든 필드를 한 번에 매핑할 수 있으며ㅡ 별도의 매핑 클래스를 사용하지 않기 때문에 코드가 간결하다는 장점이 있다
    *
    * 복합 키의 매핑에서는 복합 키 클래스에 Equals와 hashCode 메소드를 구현해야 한다는 점에 주의해야한다
    * 이는 jpa에서 엔티티 객체의 동일성을 판단하기 위해 필요하다
    * 또한 , @GeneratedValue 어노테이션을 사용하여 복합 키를 자동으로 생성하는 것은 불가능하다는 점에 주의해야한다
    *
