...
public class ManyToOneAssociationTests {
    ...
    * 예를 들어 하나의 카테고리가 여러개의 메뉴를 가질수 있는 상황에서 메뉴 엔티티가 카테고리 엔티티를 참조하는 것이다
...
// 연관 관계를 가지는 엔티티를 조회하는 방법은 객체 그래프 탐색(객체 연관관계를 사용한 조회), 객체 지향 쿼리(jpql) 사용이 있다
    ...
    * 주의할 점은 From 절에 기술할 테이블명에는 반드시 엔티티명이 작성되어야한다
    * */
    @Test
    ...
    /*
    * Commit()을 할 경우 컨텍스트 내에 저장된 영속성 객체를 insert하는 쿼리가 동작한다
    * 단, 카테고리가 존재하는 값이 아니므로 부모 테이블(tbl_category)에 값이 먼저 들어 있어야 그 카테고리를 참조하는 자식 테이블에 데이터를 넣을 수 있다
    ...
    * */
    ...
    void 영속성_삭제_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,100037);
        ...
        transaction.commit();
        MenuAndCategory deletMenu = entityManager.find(MenuAndCategory.class,100037);
    ...
    void Merge_insert_테스트(){
        ...
        EntityTransaction entityTransaction = entityManager.getTransaction();
        entityTransaction.begin();
        MenuAndCategory mergeMenu = entityManager.merge(menuAndCategory);  // 영속성 컨텍스트에서 pk 값을 자동 설정해서 merge 한다.
        System.out.println("================");
    ...
    void Merge_update_테스트(){
        ...
        Category category = entityManager.find(Category.class,33338);
        category.setCategoryName("merge카테고리 업데이트");
