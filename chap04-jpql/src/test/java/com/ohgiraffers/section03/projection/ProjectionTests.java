...
public class ProjectionTests {
    ...
    *   엔티티와 거의 비슷하게 사용되며 조회의 시작점이 될 수 없다. <FROM 절에서 사용불가>
        ...
        *
      3. 스칼라 타입 프로젝션
      *  숫자, 문자, 날짜같은 기본데이터 타입이다
      * 스칼라 타입은 영속성 컨텍스트에서 관리하지 않는다
      *
      * 4. new 명령어를 활용한 프로젝션
      *  다양한 종류의 단순 값들을 DTO로 바로 조회하는 방식으로 new 패키지명. dto명을 쓰면 해당DTO로 바로 반환 받을 수 있다
    ...
    * */
    ...
    /*
    * 조회하려는 컬럼 값이 1개인 경우 TypedQuery로 반환타입을 단일 값에 대해 지정할 수 있지만 다중 열 컬럼을 조회하는 경우 타입을 지정하지 못한다
    * 그 때 typedQuery 대신 query를 사용하여 Object[] 로 행의 정보를 받아 사용한다.
    * */
