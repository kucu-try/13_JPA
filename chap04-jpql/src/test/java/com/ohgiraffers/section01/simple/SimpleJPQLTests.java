...
public class SimpleJPQLTests {
    ...
    * 2. 쿼리객체는 "TypedQuery", "Query" 두가지가 있다.
    * - TypedQuery : 반환할 타입을 명확하게 지정하는 방식일 때 사용하며 쿼리 객체의 메소드 실행결과로 지정한 타입이 반환된다
    * -Query : 반환할 타입을 명확하게 지정할 수 없을 때 사용하며 쿼리 객체 메소드의 실행결과로 Object 또는 Object[] 가 반환 된다
    * 3. 쿼리 객체에서 제공하는 메소드 "getSingleResult()" 또는 "getResultList()"를 호출해서 쿼리를 실행하고 데이터베이스를 조회한다
    * getSingelResult() : 결과가 정확히 한 행일 경우 사용하며 없거나 많으면 예외가 발생한다
    * getResultList() : 결과가 2행 이상일 경우 사용하며 컬렉션을 반환한다. 결과가 없으면 빈 컬렉션을 반환한다.
    * */
    @Test
    public void Queryf를_이용한_단일메뉴_조회_테스트(){
    ...
    @Test
    public void Queryf를_이용한_단중행_조회_테스트(){
    ...
    @Test
    void TypedQuery를_이용한_단중행_조회_테스트(){
        //when
